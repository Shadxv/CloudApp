...
public class PacketDataManager {
    ...
    @Getter private final ByteBuf source;

    public PacketDataManager(ByteBuf source) {
        this.source = source;
    }

    public ByteBuf resetIndex(){
    ...
    public PacketDataManager writeUtf(String s) {
        return this.writeUtf(s, 32767);
    }

    public PacketDataManager writeUtf(String s, int i) {
        if (s.length() > i) {
            int j = s.length();
